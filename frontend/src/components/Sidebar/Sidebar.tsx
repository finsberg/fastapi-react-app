...
const LinkItems: Array<LinkItemProps> = [
  { name: 'Home', icon: FiHome, endpoint: '/' },
  { name: 'Todos', icon: FcTodoList, endpoint: '/todo' },
  { name: 'Users', icon: FiUsers, endpoint: '/users' },
]
    ...
    >
      ...
      {LinkItems.map((link) => (
        <NavItem
          key={link.name}
          icon={link.icon}
          onClick={() => navigate(link.endpoint, { replace: true })}
        >
          {link.name}
        </NavItem>
      ))}
